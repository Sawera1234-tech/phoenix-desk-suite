    ...
    <AppShell title="Categories" subtitle="Product Classification">
      <div className="mx-auto max-w-[1200px] space-y-4 p-6 xl:p-8">
        <DataTable<Row>
          ...
          actions={<NewDialog onCreated={() => qc.invalidateQueries({ queryKey: ["categories"] })} />}
...
function NewDialog({ onCreated }: { onCreated: () => void }) {
  ...
  const create = useMutation({
    ...
    onSuccess: () => { toast.success("Category created"); setOpen(false); setName(""); onCreated(); },
    onError: (e: Error) => toast.error(e.message),
  });
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild><Button size="sm" className="gap-1.5"><Plus className="h-4 w-4" /> New Category</Button></DialogTrigger>
      <DialogContent>
